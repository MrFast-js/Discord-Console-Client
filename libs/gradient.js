...
function InitGradient(...args) {
	...
	ret.fullConsole = (str, opts) => fullConsoleGradient(str ? str.toString() : '', grad, opts);
	return ret;
}
...
function fullConsoleGradient(str, gradient, opts) {
    let on = true
    const options = validateOptions(opts);
    const lines = str.split('\n');
...
for (const a in aliases) { // eslint-disable-line guard-for-in
	...
	module.exports[a].fullConsole = str => new InitGradient(aliases[a].colors).fullConsole(str, aliases[a].options);
}